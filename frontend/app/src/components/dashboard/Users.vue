        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div class="flex gap-4">
            ...
            <div class="bg-gray-700 px-4 py-2 rounded-lg text-center">
              ...
              <p class="text-xl font-bold text-indigo-400">{{ users.filter(u => u.role === 'admin').length }}</p>
            </div>
            <div class="bg-gray-700 px-4 py-2 rounded-lg text-center">
              <p class="text-sm text-gray-300">Writer</p>
              <p class="text-xl font-bold text-green-400">{{ users.filter(u => u.role === 'writer').length }}</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <table class="min-w-full divide-y divide-gray-700">
            <thead class="bg-gray-700">
              <tr>
                ...
                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Ruolo</th>
                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Last Login</th>
                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Azioni</th>
              </tr>
            </thead>
            <tbody class="bg-gray-800 divide-y divide-gray-700">
              <tr v-for="user in filteredUsers()" :key="user.id" class="hover:bg-gray-750 transition-colors">
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    <div class="flex-shrink-0 h-10 w-10 bg-indigo-900 rounded-full flex items-center justify-center">
                      <span class="text-indigo-300">{{ user.displayname.charAt(0).toUpperCase() }}</span>
                    </div>
                    <div class="ml-4">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-400">{{ formatDate(user.lastlogin) }}</td>
